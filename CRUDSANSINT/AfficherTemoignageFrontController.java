 ...
 * @author user
 */
public class AfficherTemoignageFrontController implements Initializable {
...
//    private Button btnRetour;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        TemoignageService fs =null;
        try {
            fs = new TemoignageService();
        } catch (SQLException ex) {
            Logger.getLogger(AfficherTemoignageFrontController.class.getName()).log(Level.SEVERE, null, ex);
        }
        List<Temoignage> us = null;
        try {
...
// calcul total unread messages !!! ( think about refresh after reply )
      ...
      //  String nbtext = Long.toString(nb);
        Label nbUnreadMessages = new Label("Temoignages:");
...
//        nbUnreadMessages.setPadding(new Insets(15, 20, 20, 20));
        ...
        for (int i = 0; i < us.size(); i++) {
            ...
            for (int j = 0; j < 3; j++) {
                if (iterator.hasNext()) {
                    vbm.getChildren().add(createVBMessage((Temoignage) iterator.next()));
                }
            }
    ...
    public VBox createVBMessage(Temoignage m) {
        ...
        vbmsg.setSpacing(10);
         ImageView img = new ImageView(new Image(m.getImageT())); //f.getPicturePath()
        ...
        nomSender.setFont(Font.font("Amble CN", FontWeight.BOLD, 20));
          Label messageBody = new Label(m.getChamp());
